...
class PyConv1(nn.Module):
    ...
    def __init__(self, in_plans, out_planes,kernel_size=3, stride=1, padding=1, dilation=1, groups=1):
        super(PyConv1, self).__init__()
        self.conv2_1 = conv(in_plans, out_planes, kernel_size=kernel_size, padding=padding,
                            stride=stride,dilation = dilation, groups=groups)

        self.cot2_1 = CoTNetLayer(dim=out_planes, kernel_size=3)

    def forward(self, x):
        x1 = self.conv2_1(x)
        return self.cot2_1(x1)
...
class PyConv4(nn.Module):
    ...
    def __init__(self, inplans, planes, pyconv_kernels=[3, 5, 7, 9], stride=1, pyconv_groups=[1, 4, 8, 16]):
        ...
        self.cot2_1 = CoTNetLayer(dim=planes, kernel_size=3)
...
class PyConvHGResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=42, zero_init_residual=False, norm_layer=None, dropout_prob0=0.0):
