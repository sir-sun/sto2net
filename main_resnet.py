...
for idx in range(1,6):
    ...
    best_acc_list=[]
    result =str(idx)
    train_data = MyDataset(txt=r'D:\PDR\Origin\Train&Test\ID_5fold\Train_Data' + result + '.txt',
                           transform=transforms.ToTensor())  # 训练集
    train_loader = DataLoader(dataset=train_data, batch_size=8, shuffle=True,
                              num_workers=0)  # 一次一起训练batch_size个样本，计算平均损失函数值，更新参数
    test_data = MyDataset(txt=r'D:\PDR\Origin\Train&Test\ID_5fold\Test_Data' + result + '.txt',
                          transform=transforms.ToTensor())  # 测试集
    ...
    def train(ep):
        ...
        fp = open(r'D:\PDR\FR\data\resnet2\Result_train_'+result + '.txt', 'a+')  # 结果存入train_result文本文件
    ...
    def test():
        ...
        with torch.no_grad():
            ...
            fp = open(r'D:\PDR\FR\data\resnet2\Result_test_' +result+ '.txt', 'a+')  # 结果存入test_result文本文件
    ...
    if __name__ == "__main__":
        ...
        torch.save(model.state_dict(), r'D:\PDR\FR\data\resnet2\\'+result + '.pth')
        ...
        #                                         title='ME_result confusion matrix')
        #
        ...
        fp = open(r'D:\PDR\FR\data\resnet2\\' + result+'.txt', 'a+')  #结果存入result4文本文件
